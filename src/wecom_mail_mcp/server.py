...
StartTimestamp = Annotated[int, Field(description="开始时间，Unix 时间戳（秒）")]
EndTimestamp = Annotated[int, Field(description="结束时间，Unix 时间戳（秒）")]
RemindBeforeMins = Annotated[int, Field(description="提前提醒分钟数，默认 15 分钟")]
MeetingAdminUserid = Annotated[str, Field(description="会议管理员的企业微信 userid")]
MeetingRoomId = Annotated[int, Field(description="会议室 ID，可通过 list_meeting_rooms 获取")]
BookerUserid = Annotated[str, Field(description="预定人的企业微信 userid")]
...
def create_server(settings: Settings) -> FastMCP[AppState]:
    ...
    @mcp.tool(
        ...
        description=(
            "预定会议室。成功后返回 schedule_id 和 booking_id，"
            "取消预定时需要这两个 ID（schedule_id 作为 meeting_id 传入）。"
        ),
    )
    async def book_meeting_room(
        ...
        schedule_id, booking_id = await state.client.book_meeting_room(request)
    ...
    @mcp.tool(
        ...
        description=(
            "取消会议室预定。需要传入预定时返回的 schedule_id（作为 meeting_id）和 booking_id。"
        ),
    )
    async def cancel_room_booking(
        meeting_id: Annotated[str, Field(description="预定时返回的 schedule_id")],
        booking_id: Annotated[str, Field(description="预定时返回的 booking_id")],
        ctx: Context,
    ) -> CancelRoomBookingResult:
        ...
        await state.client.cancel_room_booking(
